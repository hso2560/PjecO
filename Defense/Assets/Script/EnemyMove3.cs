...
public class EnemyMove3 : MonoBehaviour
{
    ...
    [Header("적 최대 HP")] [SerializeField] protected int startHp = 350;
    protected int hp;
    [SerializeField] int hp2 = 500;  //3 4 5웨이브 때의 적 체력
    ...
    protected void Update()
    {
        if (Time.timeScale > 0)
        {
            ...
            {
                animator.Play("right00");
            }
            if (transform.position.x > wayPoints[currentIndex].position.x)
            {
                animator.Play("left00");
            }
    ...
    public void Despawn()
    {
        ...
        Pool3.instance.InsertQueue(gameObject);
    }
